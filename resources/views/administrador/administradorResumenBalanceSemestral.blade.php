...
<li><a href="">Resumen Semestral</a></li>
@endsection
...
<div class="col-lg-12 ">
    <div class="card center-block">
        <div class="card-header">
            <strong class="card-title">Resumen Estados Financieros Semestrales</strong>
        </div>
        <div class="card-body card-block">
            <form action="{{route('excelmensual')}}" method="POST" enctype="multipart/form-data" class="form-horizontal">
                ...
                <div class="form-group row" >
                    <div class="col-5 col-md-4">
                        <select name="semestre" id="semestre" class="form-control-sm form-control" required style="display:none;">
...
<script type="text/javascript">
    $('#anio').on('change', function(e) {
        ...
        $.get('/Semestre/' + codaniosemestre, function(data) {
            console.log(data);
            $('#semestre').show();
            $('#semestre').empty();
...
<script type="text/javascript">
    $('#semestre').on('change', function(e) {
        ...
        var codigo = "";
        $('#tabledata').empty();
        $.get('/conosinresumensemestral/' + codaniosemestre, function(data) {
            var i = 1;
            console.log(data);
            if (data.length > 0) {
                $.each(data, function(index, datos) {
                    $('#tabledata').append('<tr><th>Cuenta</th><th>' + datos.nomsemestre + ' ' + datos.nomanio + '</th></tr>' +
                ...
                $('#tabledata').append('<tr><td><h3>No hay datos para este semestre</h3></td></tr>');
            }
        });
    });
</script>
