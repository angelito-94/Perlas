            ...
            <div class="col-xs-4">
                <div class="form-group">
                    <select class="form-control select2" style="width: 100%;" name="periodos" id="periodos">
                        ...
                        <option value="{{ $anio['codaniosemestral']}}">{!!$anio['nomsemestre'].' '.$anio['nomanio']!!}</option>
                        @endforeach
                    </select>
                </div>
            </div>
...
<script type="text/javascript">
    $('#periodos').on('change', function(e) {
        ...
        if ($(this).children('option:first-child').is(':selected')) {
            ...
            $.get('/indicadoress/' + codaniosemestre, function(data) {
                $(document).ready(function() {
                    console.log(data);
                    $('#tabledata').empty();
                    $.each(data, function(index, periodosobj) {
                        $('#tabledata').append("<tr><th>Código</th><th>Indicador</th><th>Formula</th><th style='width:10%;'>Valor " + periodosobj.nomsemestre2 + ' ' + periodosobj.nomanio2 + "</th><th style='width:10%;'>Valor " + periodosobj.nomsemestre + ' ' + periodosobj.nomanio + "</th><th style='width:10%;'>Valoración Porcentual Año " + periodosobj.nomanio + "</th><th>Información</th></tr>");
                        return false;
                    });
                    $.each(data, function(index, periodosobj) {
