...
<div class="col-lg-12 ">
    <div class="card center-block">
        <div class="card-header">
            <strong class="card-title">Estado Financiero Semestral</strong>
        </div>
        <div class="card-body card-block">
            <form action="{{route('excelsemestral')}}" method="POST" enctype="multipart/form-data" class="form-horizontal">
                {{ csrf_field() }}
                ...
                <div class="form-group row">
                    <div class="col-5 col-md-4">
                        <select name="semestre" id="semestre" class="form-control-sm form-control" required>
...
<script type="text/javascript">
    $('#anio').on('change', function(e) {
        ...
        $.get('/Semestre/' + codanio, function(data) {
            console.log(data);
            $('#semestre').empty();
            $('#semestre').append('<option value="">--Seleccione el semestre</option>');
            $.each(data, function(index, datos) {
                $('#semestre').append('<option value="' + datos.codaniosemestre + '">' + datos.nomsemestre + '</option>');
            })
        });
    });
</script>
...
<script type="text/javascript">
    $('#semestre').on('change', function(e) {
        var codaniosemestre = e.target.value;
        var codigo = "";
        $('#tabledata').empty();
        $('#actualizar').empty();
        $('#subirbalance').empty();
...
<script>
    function opciones(op, codaniosemestre) {
        if (op == 1) {
            var filas = [];
            var cont = 1;
            ...
            $.ajax({
                type: 'POST',
                url: "{{route('actualizarbalancesemestral')}}",
                dataType: 'json',
                headers: {
                ...
                complete: function() {
                    ...
                    document.getElementById('semestre').value = "Seleccione el semestre...";
                }
            });
        } else if (op == 2) {
            ...
            $.ajax({
                url: "{{route('eliminarbalancesemestral')}}",
                data: {
                    'anio': codaniosemestre
                },
                type: 'get',
                ...
                error: function() {
                    $("#loading-screen").hide();
                    $('#alertdanger').show();
                    $('#alertdanger').html("Algo salió mal, Este estado financiero no se puede eliminar");
                    setTimeout(function() {
