...
<div class="col-lg-12 ">
    <div class="card center-block">
        <div class="card-header">
            <strong class="card-title">Estado de Resultados</strong>
        </div>
        <div class="card-body card-block">
            <form action="{{route('excelEstadoResultados')}}" method="POST" enctype="multipart/form-data" class="form-horizontal">
                {{ csrf_field() }}
        ...
        if (e.target.value == 0) {
            ...
            $.get('/conosinbalance/' + codanio, function(data) {
                ...
                if (data.length > 0) {
                    ...
                    $('#tabledata').append('<thead><tr><th>Codigo Cuenta</th><th>Nombre Cuenta</th><th>Tipo</th><th>Grupo</th><th>Valor</th><th style="display:none;"></th><th style="display:none;"></th></tr></thead><tbody>' + codigo + '</tbody>');
                    $("#tabledata").dataTable({
                        "language": {
    ...
    function opciones(op, codanio) {
        if (op == 1) {
            ...
            $('#tabledata tbody tr').each(function() {
                ...
                var anio = $(this).find('td').eq(5).text();
            ...
            $.ajax({
                url: "{{route('excelEstadoResultados')}}",
                data: {
                    'anio': codanio
                },
                type: 'get',
