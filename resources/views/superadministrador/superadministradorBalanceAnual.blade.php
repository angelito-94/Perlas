...
<div class="col-lg-12 ">
    <div class="card center-block">
        ...
        <div class="card-body card-block">
            <form action="{{route('superAdministradorexcel')}}" method="POST" enctype="multipart/form-data" class="form-horizontal">
                {{ csrf_field() }}
...
<script type="text/javascript">
    $('#anio').on('change', function(e) {
        if (e.target.value == 0) {
            ...
            $.get('/suconosinbalance/' + codanio, function(data) {
                var i = 1;
                console.log(data);  
                if (data.length > 0) {
...
<script>
    function opciones(op, codanio) {
        if (op == 1) {
            ...
            $.ajax({
                type: 'POST',
                url: "{{route('superAdministradoractualizarbalance')}}",
                dataType: 'json',
                headers: {
            ...
            $.ajax({
                url: "{{route('superAdministradoreliminarbalance')}}",
                data: {
                    'anio': codanio
                },
                type: 'get',
            ...
            $.ajax({
                url: "{{route('superAdministradorexcel')}}",
                data: {
                    'anio': codanio
                },
                type: 'get',
